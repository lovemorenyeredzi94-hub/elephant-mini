...
module.exports = {
  ...
  execute: async (conn, mek, m, { args, reply, from }) => {
    try {
      if (!fetchFn) return reply(" Fetch is not available on this runtime.");
      ...
      if (args.length === 0) {
        ...
        else return reply("‚ù? Provide text or reply to a message.\nExample: `.fancy Hello`");
      } else {
        if (!isNaN(args[0])) {
          ...
          if (args.length > 1) textToConvert = args.slice(1).join(" ");
            ...
            else return reply("‚ù? No previous text found in this chat. Use `.fancy <text>` first.");
          }
        } else {
      ...
      if (!textToConvert) return reply("‚ö†Ô∏è Could not determine text.");
      ...
      if (!res.ok) return reply(" Failed to fetch fonts from API.");
      const data = await res.json();

      if (!data || !Array.isArray(data.results)) {
        return reply(" API returned no fonts.");
      }
      ...
      if (styleNumber !== null) {
        if (styleNumber < 1 || styleNumber > data.results.length) {
          return reply(` Invalid style. Choose between 1 and ${data.results.length}.`);
        }
      ...
      await conn.sendMessage(from || m.chat || mek.key?.remoteJid, {
        text: "‚ö†Ô∏è Error converting text. Try again later.",
        contextInfo: {
          mentionedJid: [],
          forwardingScore: 999,
          isForwarded: true,
